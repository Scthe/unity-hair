...
public class AnimLoopRandomStateMachine : StateMachineBehaviour
{
  ...
  [Tooltip("If true, next animation is random (can repeat). If false, cycles through the animations.")]
  ...
  {
    int nextAnimIdx = Random.Range(0, animCount);
    if (!playRandom)
    {
      nextAnimIdx = animator.GetInteger(blendVariableName) + 1;
      nextAnimIdx = nextAnimIdx % animCount;
    }

    animator.SetInteger(blendVariableName, nextAnimIdx);
    if (debugLogChanges)
    {
      Debug.Log($"Anim {blendVariableName}: {nextAnimIdx}");
    }
  }
}
